...
# Import formatters - handle both standalone and package imports
try:
    from .nl_formatters import (
        format_devices_list,
        format_entity_status,
        format_action_result,
        format_tv_remote_action,
    )
except ImportError:
    # Fallback for standalone module loading
    import importlib.util
    from pathlib import Path
    _formatter_path = Path(__file__).parent / "nl_formatters.py"
    spec = importlib.util.spec_from_file_location("nl_formatters", _formatter_path)
    if spec and spec.loader:
        nl_formatters = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(nl_formatters)
        format_devices_list = nl_formatters.format_devices_list
        format_entity_status = nl_formatters.format_entity_status
        format_action_result = nl_formatters.format_action_result
        format_tv_remote_action = nl_formatters.format_tv_remote_action
...
def HA_ACTION_tv_remote(button: str) -> str:
    ...
    Returns: Natural language confirmation message

    Example Prompt: "open spotify on my tv"
    Example Response: "I've launched Spotify on Living Room TV."
    Example Args: {"button": "spotify"}
    """
    err = _require_token()
    if err:
        return err
    ...
    try:
        ...
        return format_tv_remote_action(
            button=button,
        ...
        return format_tv_remote_action(
            ...
            remote_entity=remote_entity,
            success=False,
            error_message=f"Error sending '{button}' command: {str(e)}"
        )
...
def HA_GET_devices() -> str:
    ...
    Example Response: "Found 5 devices in your home:\n\n**Lights:**\n  - Kitchen Light: on\n  - Bedroom Light: off\n\n**Switches:**\n  - Living Room Fan: on"
    Example Args: {}
    Returns a natural language description of all devices with their entity names, domains, and current states.
    """
    err = _require_token()
    if err:
        return err
    try:
        ...
        for state in states:
            try:
                ...
                continue
        return format_devices_list(devices)
    except Exception as e:  # pragma: no cover - best-effort
        return f"Error listing devices: {str(e)}"


def HA_GET_entity_status(entity_id: Optional[str] = None, friendly_name: Optional[str] = None, entity_name: Optional[str] = None) -> str:
    """Get status of a specific Home Assistant entity.
    Example Prompt: "what's the status of the living room light?"
    Example Response: "The Living Room Light is on."
    ...
    Returns a natural language description of the entity's current state.
    """
    err = _require_token()
    if err:
        return err
    ...
    if not isinstance(identifier, str) or not identifier.strip():
        return "Missing 'entity_id' or 'friendly_name'"
...
def HA_ACTION_turn_entity_on(entity_id: Optional[str] = None, friendly_name: Optional[str] = None, entity_name: Optional[str] = None) -> str:
    """Turn on a specific Home Assistant entity.
    Example Prompt: "turn on the kitchen light"
    Example Response: "I've turned on the Kitchen Light."
    Example Args: {"entity_id": "string[entity id]"} or {"friendly_name": "string[name]"}
    Accepts either a full `entity_id` (e.g. 'light.kitchen') or a `friendly_name`
    (e.g. 'Kitchen Light'). Also accepts `entity_name` as an alias for `friendly_name`.
    Returns a natural language confirmation message.
    """
    err = _require_token()
    if err:
        return err
    ...
    # Get friendly name before action
    try:
        state_url = f"{HA_URL}/api/states/{entity_id}"
        state_resp = requests.get(state_url, headers=_headers(), timeout=10)
        fname = None
    ...
    try:
        ...
        return format_action_result(
            ...
            action="turn_on",
            success=False,
            friendly_name=fname if 'fname' in locals() else entity_id,
            error_message=f"Error turning on {fname if 'fname' in locals() else entity_id}: {str(e)}"
        )


def HA_ACTION_turn_entity_off(entity_id: Optional[str] = None, friendly_name: Optional[str] = None, entity_name: Optional[str] = None) -> str:
    """Turn off a specific Home Assistant entity.
    Example Prompt: "turn off the kitchen light"
    Example Response: "I've turned off the Kitchen Light."
    Example Args: {"entity_id": "string[entity id]"} or {"friendly_name": "string[name]"}
    Accepts either a full `entity_id` (e.g. 'light.kitchen') or a `friendly_name`
    (e.g. 'Kitchen Light'). Also accepts `entity_name` as an alias for `friendly_name`.
    Returns a natural language confirmation message.
    ...
    try:
        ...
        return format_action_result(
            ...
            success=False,
            friendly_name=fname if 'fname' in locals() else entity_id,
            error_message=f"Error turning off {fname if 'fname' in locals() else entity_id}: {str(e)}"
        )
