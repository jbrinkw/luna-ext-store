...
Hosted in a `luna-extensions` monorepo with a single `registry.json` that now advertises both extensions and infrastructure services.

  * **Top-Level Schema:**
    ```json
    {
      "version": "1.1",
      "extensions": [ /* extension metadata (legacy format retained) */ ],
      "categories": [/* optional category strings or {id,name} objects */],
      "external_services": [
        {
          "name": "grocy",
          "display_name": "Grocy Household Manager",
          "description": "Self-hosted groceries, chores, and household management dashboard.",
          "category": "productivity",
          "tags": ["docker", "inventory"],
          "version": "linuxserver-3.3.2",
          "repository": "https://github.com/linuxserver/docker-grocy",
          "path": "services/grocy",
          "service_definition": { /* optional inline service.json (preferred) */ },
          "service_definition_url": "https://.../grocy.json",  /* fallback download URL */
          "provides_vars": ["GROCY_URL", "GROCY_PORT"],        /* duplicated for quick preview */
          "config_fields": 3,                                 /* optional summary hint */
          "ui": { "base_path": "ext_service", "slug": "grocy", "port_field": "port", "open_mode": "iframe" }
        }
      ]
    }
    ```

  * **Extensions Workflow:** Unchanged. The Hub calls `POST /api/extensions/install` (or toggles pending installs in `master_config`) and queues changes for `apply_updates.py`.
  * **External Services Workflow:**
      1. Store UI shows external services in a dedicated tab, using registry metadata for filtering and badges (UI availability, env vars, config fields).
      2. Selecting “Add to Luna” either uploads the inline `service_definition` or downloads the `service_definition_url`, then POSTs it to `POST /api/external-services/upload`.
      3. Once uploaded, the service appears under `external_services/` and in the supervisor catalog; users complete installation from the Infrastructure page (`POST /api/external-services/{name}/install`).
      4. Entries that already exist locally display a “Manage” shortcut which deep-links to `/infrastructure`.
